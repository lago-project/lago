...
class NATNetwork(Network):
    def _libvirt_xml(self):
        ...
        replacements = {
            ...
            '@GW_ADDR@': self.gw(),
	    '@DNS_DOMAIN_NAME@': self._spec.get('dns_domain_name', 'lago.example.com'),
        }
        ...
        if 'dhcp' in self._spec:
            ...
            dhcp_v6 = lxml.etree.Element('dhcp')
            ipv4.append(dhcp)
            ipv6.append(dhcp_v6)
        ...
        for dev_spec in self._spec['nics']:
            ...
            if 'ip' in dev_spec:
                interface.append(
                    lxml.etree.Element(
                        'mac',
                        address=_ipv4_to_mac(dev_spec['ip'])
                    ),
                )
