...
def _get_cipher(self, name, key, iv):
    """
    Workaround for BZ: https://bugzilla.redhat.com/show_bug.cgi?id=1419312
    This monkeypatching was tested to resolve the issue with the combination
    of:
    paramiko-1.16.1
    python2-crypto-2.6.1-13.el7
    """
    from paramiko.transport import Counter, SSHException, util
    ...
    if name in ('arcfour128', 'arcfour256'):
        ...
        counter = Counter.new(
            nbits=self._cipher_info[name]['block-size'] * 8,
            initial_value=util.inflate_long(iv, True)
        )
        return self._cipher_info[name]['class'].new(
            key, self._cipher_info[name]['mode'], '', counter
        )
    else:
        return self._cipher_info[name]['class'].new(
            key, self._cipher_info[name]['mode'], iv
        )


if paramiko.__version__ == '1.16.1':
    LOGGER.debug('monkeypatching paramiko.transport.Transport._get_cipher')
    paramiko.transport.Transport._get_cipher = _get_cipher
